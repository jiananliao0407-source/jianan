...
<head>
  ...
  <style>
    ...
    .bar { width: 140px; height: 10px; border-radius: 999px; background: rgba(255,255,255,0.10);
...
<body>
  <div class="wrap" id="wrap-$reset_key">
    ...
    <canvas id="c" tabindex="0" width="760" height="360"></canvas>
...
<script>
(() => {
  const DIFFICULTY0 = $difficulty;
  const SPEED_MULT  = $speed_mult;
  const GRAVITY_MULT = $gravity_mult;

  const canvas = document.getElementById('c');
  const ctx = canvas.getContext('2d');

  // Ensure Space works: focus canvas + stop page scrolling
  ...
  const keys = new Set();
  function pressJump(){ player.jumpBuffer = 0.12; }
  function pressDash(){
    if (!running) return;
    if (player.dashCD <= 0) { player.dashT = 0.14; player.dashCD = 0.9; }
  }
  ...
  // Attach to canvas so focus matters (no random page scroll)
  ...
  function draw(){
    ...
    // platforms
    for (const p of platforms){
      ctx.fillStyle = p.moving ? "rgba(160,130,255,0.18)" : "rgba(120,170,255,0.18)";
    ...
    if (!running){
      ...
      ctx.fillText("Press R to restart", w/2 - 70, h/2 + 22);
    }
  }
...
""").substitute(
    ...
    speed_mult=str(float(speed_mult)),
    gravity_mult=str(float(gravity_mult)),
)

# Extra-safe: render HTML via base64 iframe so Streamlit doesn't mangle it
